...
def generate_dataset_with_relevant_corpus(split: str, qrels_path: str):
    """
...
def generate_dataest_with_full_corpus(split, qrels_path: str):
    with open(qrels_path) as f:
        qrels = f.readlines()
    qrels = [q.split() for q in qrels[1:]]
    print(f'#{split}', len(qrels))
...
if __name__ == '__main__':
    ...
    if os.path.isfile(os.path.join(args.data, 'qrels/test.tsv')):  # test set
        if args.corpus == 'relevant':
            generate_dataset_with_relevant_corpus('test', os.path.join(args.data, 'qrels/test.tsv'))
        elif args.corpus == 'full':
            generate_dataest_with_full_corpus('test', os.path.join(args.data, 'qrels/test.tsv'))
